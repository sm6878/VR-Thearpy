...
public class SpeechRecognition : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        SpeechServiceAPIKey = "66c046cc840041a0bb528a1fd8c54362";
...
#region Speech Recognition event handlers
    ...
    private void RecognizingHandler(object sender, SpeechRecognitionEventArgs e)
    {
        if (e.Result.Reason == ResultReason.RecognizingSpeech)
        {
            ...
            {
                res = "Hi friend,I'm doing great What about you.";
            }
            else if (res.Contains("good") || res.Contains("better") || res.Contains("great"))
            {
                res = "That is a sign of progress. Let's talk more about it";
            }
            else if (res.Contains("wrong") || res.Contains("failed") || res.Contains("incorrect"))
            {
                res = "All of us face failure at one time or another.Its a part of life";
            }
            else if (res.Contains("depressed") || res.Contains("anxiety") || res.Contains("worried"))
            {
                res = "You’re not alone in this. I may not understand exactly how you feel, but I care about you and want to help";
            }
            else if (res.Contains("help") || res.Contains("share") || res.Contains("protect"))
            {
                res = "Asking for help is a sign of strength.Don't worry I am here to help";
            }
            else if (res.Contains("bad") || res.Contains("sad") || res.Contains("pain") || res.Contains("low"))
            {
                res = "Ups and Downs are part of life. You can share your worries with me.";
            }
            else if (res.Contains("fear") || res.Contains("afraid") || res.Contains("terrified"))
            {
                res = "Don't worry.You are safe. I am here with you.Everything will be alright";
            }
            else if (res.Contains("escape") || res.Contains("die") || res.Contains("enough"))
            {
                res = "It may look dark right now. But night is the darkest just before the dawn.Don't worry everything will be alright";
            }
            else if (res.Contains("thanks") || res.Contains("thankyou") || res.Contains("helpful"))
            {
                res = "Anytime my friend.Always there for you.";
            }
            ...
            else
            {
                ...
                //{
                //  vari = !vari;
                res = "Ok,Please tell me more.Don't Hesitate I'm your friend";
                //}
                //else
                //{
                //    vari = !vari;
                //    res = "Oh ok. great going";
                //}
            }
            lock (threadLocker)
            {
                recognizedString = $"You: {e.Result.Text}{Environment.NewLine}Therapist: {res}";
            }
        }
    }
    ...
    private void RecognizedHandler(object sender, SpeechRecognitionEventArgs e)
    {
        if (e.Result.Reason == ResultReason.RecognizedSpeech)
        {
            ...
            if (res.Contains("hello") || res.Contains("hi"))
            {
                res = "Hello friend,I am doing great.What about you?";
            }
            else if (res.Contains("good") || res.Contains("better") || res.Contains("great"))
            {
                res = "That is a sign of progress. Let's talk more about it";
            ...
            {
                res = "That's great! Why do you think you did it?";
            }
            else if (res.Contains("bad") || res.Contains("sad") || res.Contains("pain"))
            {
                res = "Ups and Downs are part of life. You can share your worries with me.";
            }
            else if (res.Contains("fear") || res.Contains("afraid") || res.Contains("terrified"))
            {
                res = "You are safe. I am here with you. It will be alright";
            }
            else if (res.Contains("escape") || res.Contains("die") || res.Contains("enough"))
            {
                res = "It may look dark right now. But night is the darkest just before the dawn";
            }
            else
            {
                //if (vari)
...
#region Speech Translation event handlers
    ...
    private void RecognizingTranslationHandler(object sender, TranslationRecognitionEventArgs e)
    {
        if (e.Result.Reason == ResultReason.TranslatingSpeech)
        {
            ...
            lock (threadLocker)
            {
                recognizedString = $"Therapist ({fromLanguage}): {e.Result.Text}";
                recognizedString += $"{Environment.NewLine}TRANSLATED Therapist Response:";
    ...
    private void RecognizedTranslationHandler(object sender, TranslationRecognitionEventArgs e)
    {
        ...
        {
            UnityEngine.Debug.LogFormat($"Therapist: {e.Result.Text}");
            lock (threadLocker)
            {
                recognizedString = $"Therapist: {e.Result.Text}";
            }
        }
        else if (e.Result.Reason == ResultReason.NoMatch)
        {
